...
class Blade2DCamberThickness:
    ...
    def get_section_coordinates(self, u):
        ...
        u_lower = jnp.sort((u[(u >= 0.00) & (u < 0.50)] - 0.00) / (0.50 - 0.00))
        u_upper = jnp.sort((u[(u >= 0.50) & (u <= 1.00)] - 0.50) / (1.00 - 0.50))
        ...
        section_coordinates = jnp.concatenate((lower_surface_coordinates, upper_surface_coordinates), axis=1)
    ...
    def make_camberline(self):
        ...
        x0 = self.x_in
        y0 = self.y_in
        x1 = self.x_in + self.dist_in * jnp.cos(self.theta_in)
        y1 = self.y_in + self.dist_in * jnp.sin(self.theta_in)
        x3 = self.x_in + self.chord * jnp.cos(self.stagger)
        y3 = self.x_in + self.chord * jnp.sin(self.stagger)
        x2 = x3 - self.dist_out * jnp.cos(self.theta_out)
        y2 = y3 - self.dist_out * jnp.sin(self.theta_out)

        # Array of control points
        P = jnp.asarray([[x0, y0], [x1, y1], [x2, y2], [x3, y3]]).transpose()
    ...
    def get_camberline_normal(self, u):
        ...
        tangent = dr_sample / jnp.sum(dr_sample ** 2, axis=0) ** (1 / 2)
        camberline_normal = jnp.asarray([-tangent[1, :], tangent[0, :]])
    ...
    def make_upper_thickness_distribution(self):
        ...
        # Array of control points
        P = jnp.asarray([self.thickness_upper])
    ...
    def make_lower_thickness_distribution(self):
        ...
        # Array of control points
        P = jnp.asarray([self.thickness_lower])
    ...
    def get_sampling_points(self, sampling_mode='uniform'):
        ...
        if sampling_mode == 'uniform':
            u_sample = jnp.linspace(0, 1, self.N_sample)

        elif sampling_mode == 'cosine':
            u = jnp.linspace(0, 1, self.N_sample)
            u_sample = jnp.cos(jnp.pi / 2 * (1 - u)) ** 2

        elif sampling_mode == 'cluster':
            u = jnp.linspace(1e-6, 1 - 1e-6, self.N_sample)
    ...
    def make_upper_side(self):
        ...
        P = jnp.concatenate((P[:, 0, jnp.newaxis], P1, P[:, 1:-1], P2, P[:, -1, jnp.newaxis]), axis=1)
    ...
    def get_start_G2_control_point(self, P, p, U, normal, curvature):
        ...
        P0 = P[:, 0]
        P2 = P[:, 1]
        # dist_02 = jnp.linalg.norm(P2 - P0)
        dist_02 = jnp.sum((P2-P0)**2)**(1/2)
        alpha = jnp.arccos(jnp.dot(P2 - P0, normal) / dist_02)
        dist_01 = jnp.sqrt(b_2 / a_1 ** 2 / curvature * dist_02 * jnp.sin(alpha))
    ...
    def check_analytic_curvature(self):
        ...
        upper_curvature = jnp.real(jnp.abs(jnp.cross(ddr, dr, axisa=0, axisb=0)) / (jnp.sum(dr ** 2, axis=0)) ** (3 / 2))
        ...
        lower_curvature = jnp.real(jnp.abs(jnp.cross(ddr, dr, axisa=0, axisb=0)) / (jnp.sum(dr ** 2, axis=0)) ** (3 / 2))
    ...
    def plot_blade_section(self, fig=None, ax=None,
        ...
        # Plot camber line
        if camberline == 'yes':
            camberline_coordinates = jnp.real(self.get_camberline_coordinates(u_plot))
        ...
        # Draw upper side
        if upper_side == 'yes':
            upper_surface_coordinates = jnp.real(self.get_upper_side_coordinates(u_plot))
        ...
        # Draw lower side
        if lower_side == 'yes':
            lower_surface_coordinates = jnp.real(self.get_lower_side_coordinates(u_plot))
        ...
        if lower_side_control_points == 'yes':
            line, = ax.plot(jnp.real(self.lower_side_BSpline.P[0, :]), jnp.real(self.lower_side_BSpline.P[1, :]))
            line.set_linewidth(0.75)
            line.set_linestyle("-.")
            line.set_color("r")
            line.set_marker("o")
        ...
        if camberline_sample_points == 'yes':
            line, = ax.plot(jnp.real(self.camberline_sample_points[0, :]), jnp.real(self.camberline_sample_points[1, :]))
        ...
        if camberline_control_points == 'yes':
            line, = ax.plot(jnp.real(self.camberline.P[1, :]), jnp.real(self.camberline.P[1, :]))
            line.set_linewidth(0.75)
            line.set_linestyle("-.")
            line.set_color("r")
            line.set_marker("o")
        ...
        if leading_edge_radius == 'yes':
            ...
            x_c = self.x_in + self.radius_in * jnp.cos(self.theta_in)
            y_c = self.y_in + self.radius_in * jnp.sin(self.theta_in)
        ...
        if trailing_edge_radius == 'yes':
            # Get the circle coordinates
            theta = jnp.linspace(0, 2 * jnp.pi, 250)
            x_c = self.x_in + self.chord * jnp.cos(self.stagger) - self.radius_out * jnp.cos(self.theta_out)
            y_c = self.y_in + self.chord * jnp.sin(self.stagger) - self.radius_out * jnp.sin(self.theta_out)
    ...
    def plot_blade_cascade(self, fig=None, ax=None):
        ...
        for k in range(3):
            ...
            # Draw upper surface
            section_coordinates = jnp.real(self.get_section_coordinates(u_plot))
    ...
    def plot_curvature_distribution(self, fig=None, ax=None):
        ...
        h = 1e-5
        hh = h + h**2
        u_upper = jnp.linspace(0.00 + hh, 0.50 - hh, 1000)
        u_lower = jnp.linspace(0.50 + hh, 1.00 - hh, 1000)

        # Plot upper surface curvature
        line, = ax.plot(2*u_upper, jnp.real(self.get_section_curvature(u_upper)))
        line.set_linewidth(1.25)
        line.set_linestyle("-")
        line.set_color("b")
        line.set_marker(" ")
        ...
        line, = ax.plot(2*u_upper[::-1], jnp.real(self.get_section_curvature(u_lower)))
        line.set_linewidth(1.25)
        line.set_linestyle("-")
        line.set_color("r")
        line.set_marker(" ")
    ...
    def plot_thickness_distribution(self, fig=None, ax=None):
        ...
        line, = ax.plot(u_plot, jnp.real(self.get_upper_thickness_distribution_values(u_plot)[0, :]))
        line.set_linewidth(0.75)
        line.set_linestyle("-")
        line.set_color("k")
        line.set_marker(" ")
        ...
        line, = ax.plot(u_plot, -jnp.real(self.get_lower_thickness_distribution_values(u_plot)[0, :]))
        line.set_linewidth(0.75)
        line.set_linestyle("-")
        line.set_color("k")
        line.set_marker(" ")
        ...
        n = jnp.shape(self.upper_thickness_distribution.P)[1]
        u_thickness = jnp.linspace(0, 1, n)
        line, = ax.plot(u_thickness, jnp.real(self.upper_thickness_distribution.P[0, :]))
        line.set_linewidth(1.25)
        line.set_linestyle("-.")
        line.set_color("r")
        line.set_marker("o")
        ...
        n = jnp.shape(self.lower_thickness_distribution.P)[1]
        u_thickness = jnp.linspace(0, 1, n)
        line, = ax.plot(u_thickness, -jnp.real(self.lower_thickness_distribution.P[0, :]))
        line.set_linewidth(1.25)
        line.set_linestyle("-.")
        line.set_color("r")
        line.set_marker("o")
        ...
        line, = ax.plot(self.u_sample_points, jnp.real(self.get_upper_thickness_distribution_values(self.u_sample_points)[0, :]))
        line.set_linewidth(0.75)
        line.set_linestyle(" ")
        line.set_color("k")
        line.set_marker("o")
        ...
        line, = ax.plot(self.u_sample_points, -jnp.real(self.get_lower_thickness_distribution_values(self.u_sample_points)[0, :]))
        line.set_linewidth(0.75)
        line.set_linestyle(" ")
        line.set_color("k")
        line.set_marker("o")
