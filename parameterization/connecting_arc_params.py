...
class Blade2DConnectingArcs:
    ...
    def __init__(self, section_variables):
        ...
        self.chord = 1.00/jnp.cos(self.stagger)
        self.spacing = 0.75*self.chord

        # Compute the location of point 1
        self.theta1 = self.theta_in + self.wedge_in
        self.x1 = self.x_in + self.radius_in*(1-jnp.sin(self.theta1))
        self.y1 = self.y_in + self.radius_in*jnp.cos(self.theta1)

        # Compute the location of point 2
        self.theta2 = self.theta_in - self.wedge_in
        self.x2 = self.x_in + self.radius_in*(1+jnp.sin(self.theta2))
        self.y2 = self.y_in - self.radius_in*jnp.cos(self.theta2)

        # Compute the location of point 3
        self.theta3 = self.theta_out + self.wedge_out
        self.x3 = self.x_in + self.chord*jnp.cos(self.stagger) - self.radius_out*(1-jnp.sin(self.theta3))
        self.y3 = self.y_in + self.chord*jnp.sin(self.stagger) - self.radius_out *jnp.cos(self.theta3)

        # Compute the location of point 4
        self.theta4 = self.theta_out - self.wedge_out
        self.x4 = self.x_in + self.chord*jnp.cos(self.stagger) - self.radius_out*(1+jnp.sin(self.theta4))
        self.y4 = self.y_in + self.chord*jnp.sin(self.stagger) + self.radius_out*jnp.cos(self.theta4)
    ...
    def get_section_coordinates(self, u):
        ...
        u12 = jnp.sort((u[(u >= 0.00) & (u < 0.25)]-0.00)/(0.25-0.00))
        u23 = jnp.sort((u[(u >= 0.25) & (u < 0.50)]-0.25)/(0.50-0.25))
        u34 = jnp.sort((u[(u >= 0.50) & (u < 0.75)]-0.50)/(0.75-0.50))
        u41 = jnp.sort((u[(u >= 0.75) & (u <= 1.00)]-0.75)/(1.00-0.75))
        ...
        section_coordinates = jnp.concatenate((coordinates_12, coordinates_23, coordinates_34, coordinates_41), axis=1)
    ...
    def make_leading_edge(self):
        ...
        P0 = jnp.array([self.x1, self.y1])                       # First control point
        P2 = jnp.array([self.x2, self.y2])                       # Third control point
        n = jnp.asarray([-(P2[1] - P0[1]), (P2[0] - P0[0])])     # Normal direction
        n = n / jnp.sqrt(n[0]**2+n[1]**2)                        # Unitary normal vector
        a = self.wedge_in                                       # Circular arc semi-angle
        R = self.radius_in                                      # Circular arc radius
        P1 = (P0 + P2) / 2 - R * jnp.cos(a) / jnp.tan(a) * n      # Second control point (see notes)
        P = jnp.array([P0, P1, P2]).transpose()                  # Matrix of control points
    ...
    def make_lower_side(self):
        ...
        # Compute the control points
        P0 = jnp.array([self.x2, self.y2])
        P1 = jnp.array([self.x2, self.y2]) + self.dist_2 * self.chord * jnp.array(
            [jnp.cos(self.theta2), jnp.sin(self.theta2)])
        P2 = jnp.array([self.x3, self.y3]) - self.dist_3 * self.chord * jnp.array(
            [jnp.cos(self.theta3), jnp.sin(self.theta3)])
        P3 = jnp.array([self.x3, self.y3])
        P = jnp.array([P0, P1, P2, P3]).transpose()              # Matrix of control points
    ...
    def make_trailing_edge(self):
        ...
        # Compute the control points
        P0 = jnp.array([self.x3, self.y3])                       # First control point
        P2 = jnp.array([self.x4, self.y4])                       # Third control point
        n = jnp.asarray([-(P2[1] - P0[1]), (P2[0] - P0[0])])     # Normal direction
        n = n / jnp.sqrt(n[0]**2+n[1]**2)                        # Unitary normal vector
    ...
    def make_upper_side(self):
        ...
        # Compute the control points
        P0 = jnp.array([self.x4, self.y4])
        P1 = jnp.array([self.x4, self.y4]) - self.dist_4 * self.chord * jnp.array([jnp.cos(self.theta4), jnp.sin(self.theta4)])
        P2 = jnp.array([self.x1, self.y1]) + self.dist_1 * self.chord * jnp.array([jnp.cos(self.theta1), jnp.sin(self.theta1)])
        P3 = jnp.array([self.x1, self.y1])
        P = jnp.array([P0, P1, P2, P3]).transpose()              # Matrix of control points
    ...
    def get_section_curvature(self, u, h=1e-5):
        ...
        section_curvature = jnp.abs(jnp.cross(ddr, dr, axisa=0, axisb=0)) / (jnp.sum(dr ** 2, axis=0)) ** (3 / 2)
    ...
    def plot_blade(self, fig=None, ax=None,
        ...
        ax.axis('off')
        h = 0
        u_plot = jnp.linspace(0.00+h, 1-h, 300)

        # Plot the blade surface
        if blade_section == 'yes':

            # Plot leading edge
            line, = ax.plot(jnp.real(self.leading_edge.get_NurbsCurve_value(u_plot)[0, :]),
                            jnp.real(self.leading_edge.get_NurbsCurve_value(u_plot)[1, :]))
            ...
            # Plot lower_surface
            line, = ax.plot(jnp.real(self.lower_side.get_NurbsCurve_value(u_plot)[0, :]),
                            jnp.real(self.lower_side.get_NurbsCurve_value(u_plot)[1, :]))
            line.set_linewidth(1.25)
            line.set_linestyle("-")
            line.set_color("k")
            line.set_marker(" ")
            ...
            # Plot trailing edge
            line, = ax.plot(jnp.real(self.trailing_edge.get_NurbsCurve_value(u_plot)[0, :]),
                            jnp.real(self.trailing_edge.get_NurbsCurve_value(u_plot)[1, :]))
            line.set_linewidth(1.25)
            line.set_linestyle("-")
            line.set_color("k")
            line.set_marker(" ")
            ...
            line, = ax.plot(jnp.real(self.upper_side.get_NurbsCurve_value(u_plot)[0, :]),
                            jnp.real(self.upper_side.get_NurbsCurve_value(u_plot)[1, :]))
            line.set_linewidth(1.25)
            line.set_linestyle("-")
            line.set_color("k")
            line.set_marker(" ")
        ...
        if control_points == 'yes':
            ...
            line, = ax.plot(jnp.real(self.leading_edge.P[0, :]), jnp.real(self.leading_edge.P[1, :]))
            ...
            line, = ax.plot(jnp.real(self.lower_side.P[0, :]), jnp.real(self.lower_side.P[1, :]))
            line.set_linewidth(0.75)
            line.set_linestyle("-.")
            line.set_color("r")
            line.set_marker("o")
            ...
            line, = ax.plot(jnp.real(self.trailing_edge.P[0, :]), jnp.real(self.trailing_edge.P[1, :]))
            line.set_linewidth(0.75)
            line.set_linestyle("-.")
            line.set_color("r")
            line.set_marker("o")
            ...
            line, = ax.plot(jnp.real(self.upper_side.P[0, :]), jnp.real(self.upper_side.P[1, :]))
            line.set_linewidth(0.75)
            line.set_linestyle("-.")
            line.set_color("r")
            line.set_marker("o")
        ...
        if leading_edge_radius == 'yes':
            ...
            theta = jnp.linspace(0, 2 * jnp.pi, 250)
            x_c = self.x_in + self.radius_in
            y_c = self.y_in
            x_circle = x_c + self.radius_in * jnp.cos(theta)
            y_circle = y_c + self.radius_in * jnp.sin(theta)
        ...
        if trailing_edge_radius == 'yes':
            ...
            # Get the circle coordinates
            theta = jnp.linspace(0, 2 * jnp.pi, 250)
            x_c = self.x_in + self.chord * jnp.cos(self.stagger) - self.radius_out
            y_c = self.y_in + self.chord * jnp.sin(self.stagger)
            x_circle = x_c + self.radius_out * jnp.cos(theta)
            y_circle = y_c + self.radius_out * jnp.sin(theta)
    ...
    def plot_blade_cascade(self, fig=None, ax=None):
        ...
        # ax.set_yticks([])
        ax.axis('off')
        u_plot = jnp.linspace(0.00, 1.00, 500)

        for k in range(3):

            # Draw blade surface
            line, = ax.plot(jnp.real(self.get_section_coordinates(u_plot)[0, :]),
                            jnp.real(self.get_section_coordinates(u_plot)[1, :])+ self.spacing * (k - 1))
            line.set_linewidth(1.25)
            line.set_linestyle("-")
            line.set_color("k")
            line.set_marker(" ")
    ...
    def plot_curvature_distribution(self, fig=None, ax=None):
        ...
        h = 1e-5
        hh = h + h**2
        u12 = jnp.linspace(0.00 + hh, 0.25 - hh, 200)
        u23 = jnp.linspace(0.25 + hh, 0.50 - hh, 200)
        u34 = jnp.linspace(0.50 + hh, 0.75 - hh, 200)
        u41 = jnp.linspace(0.75 + hh, 1.00 - hh, 200)

        # Leading edge curvature
        section_curvature = jnp.real(self.get_section_curvature(u12, h))
        ...
        section_curvature = jnp.real(self.get_section_curvature(u23, h))
        ...
        section_curvature = jnp.real(self.get_section_curvature(u34, h))
        ...
        section_curvature = jnp.real(self.get_section_curvature(u41, h))
