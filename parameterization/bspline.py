...
# jax.scipy has not implemented BSplineCurves (as of September 12, 2023)
class BSplineCurve:
    ...
    def get_BSplineCurve_value(self, u):
        self.u = u
        self.Nu = 1 if jnp.isscalar(u) else u.size
    ...
    @staticmethod
    @jit
    def evaluate_BSplineCurve(P, p, U, u):
        n = P.shape[1] - 1
        N_basis = BSplineCurve.get_basis_polynomials(n, p, U, u)
        return jnp.dot(P, N_basis)

    @staticmethod
    @jit
    def get_basis_polynomials(n, p, U, u):
        assert (p <= n), 'The order of the basis polynomials cannot be larger than the number of control points minus one'
        Nu = u.size
        N = jnp.zeros((n + p + 1, Nu))
        for i in range(n + p + 1):
            n1 = jnp.where(U[i + p] - U[i] == 0, 0.0, (u - U[i]) / (U[i + p] - U[i])) * N[i, :]
            n2 = jnp.where(U[i + p + 1] - U[i + 1] == 0, 0.0, (U[i + p + 1] - u) / (U[i + p + 1] - U[i + 1])) * N[i + 1, :]
            N[i, :] = n1 + n2
        p = 1
        while Nu > 1:
            m = N.shape[0] - 1
            N_bis = jnp.zeros((m, Nu))
            for i in range(m):
                n1 = jnp.where(U[i + p] - U[i] == 0, 0.0, (u - U[i]) / (U[i + p] - U[i])) * N[i, :]
                n2 = jnp.where(U[i + p + 1] - U[i + 1] == 0, 0.0, (U[i + p + 1] - u) / (U[i + p + 1] - U[i + 1])) * N[i + 1, :]
                N_bis[i, :] = n1 + n2
    ...
    def plot_BSplineCurve(self, options, order=0):
        C, P = jnp.real(self.dC[str(order) + '-derivative']), jnp.real(self.dP[str(order) + '-derivative'])
        ndim = self.P.shape[0]
...
class NurbsCurve:
    ...
    def evaluate_NurbsCurve(self, P, W, p, U, u):
        ...
        # Pw = jnp.vstack((jnp.asarray(P*W), W))

        # Map the control points to the expanded space | P = (x*w,y*w,z*w,w)
        w = jnp.repeat(W[jnp.newaxis, :], repeats=n_dim, axis=0)
        P_w = jnp.vstack((jnp.asarray(P*w), W[jnp.newaxis, :]))

        # Compute the coordinates of the NURBS curve in the expanded space
        C_w = jnp.dot(P_w, N_basis)
        ...
        # W = jnp.repeat(W[:, jnp.newaxis], repeats=self.Nu, axis=1)
        # R_basis = (N_basis * W)/jnp.repeat(jnp.sum(N_basis*W, axis=0)[jnp.newaxis,:], repeats=n+1, axis=0)
        # C = jnp.dot(P, R_basis)
    ...
    def get_basis_polynomials(self, n, p, U, u):
        ...
        N = jnp.zeros((m, jnp.size(u)))       # Initialize the array of basis polynomials
        ...
        for i in range(m):
            N[i, :] = 0.0 + 1.0 * (u >= U[i]) * (u < U[i + 1]) + 1.00 * (jnp.logical_and(u == 1, i == n))
        ...
        while m > n + 1:
            ...
            N_bis = jnp.zeros((m, Nu), dtype=complex)
            # N_bis = jnp.zeros((m, Nu))
...
class NurbsSurface:
    ...
    def evaluate_NurbsSurface(self, P, W, p, q, U, V, u, v):
        ...
        # The implementation of the coordinates computations uses several functions for vectorized code (increase speed)
        A = jnp.dot(P_w, N_basis_v)                                      # shape (ndim+1, n+1, N)
        B = jnp.repeat(N_basis_u[jnp.newaxis], repeats=n_dim+1, axis=0)   # shape (ndim+1, n+1, N)
        S_w = jnp.sum(A*B,axis=1)                                        # shape (ndim+1, N)
    ...
    def plot_NurbsSurface(self, options):
        ...
        Px = jnp.real(self.P[0, :, :])
        Py = jnp.real(self.P[1, :, :])
        Pz = jnp.real(self.P[2, :, :])
